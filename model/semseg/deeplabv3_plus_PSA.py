...
from model.models.nets1.mobilevit_utils.PSA import PSA_p, PSA_s
...
class DeepLab_PSA(nn.Module):
    def __init__(
        ...
        super(DeepLab_PSA, self).__init__()
        ...
        self.psa_p = PSA_p(inplanes=in_channels, planes=in_channels)  # 参数可能需要根据实际情况调整
        self.psa_s = PSA_s(inplanes=in_channels, planes=in_channels)
        # -----------------------------------------#
        #   ASPP特征提取模块
        #   利用不同膨胀率的膨胀卷积进行特征提取
        # -----------------------------------------#
    ...
    def forward(self, x,need_fp=False):
        ...
        x = self.psa_p(x)  # 使用 PSA_p
        x = self.psa_s(x)  # 使用 PSA_s
...
if __name__ == '__main__':
    model = DeepLab_PSA(num_classes=3,backbone="hrnet", downsample_factor=16, pretrained=True)
