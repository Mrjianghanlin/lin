...
class DeepLabV3Plus(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg['backbone']=="resnet50":
            ...
            high_channels = 480
                    ...
                    # 直接用 480 作为输入通道
            self.align_c4 = nn.Conv2d(480, high_channels, 1, bias=False)
            self.align_c4_bn = nn.BatchNorm2d(high_channels)
            self.align_c4_relu = nn.ReLU(True)
        else:
    ...
    def forward(self, x, need_fp=False):
        ...
        if hasattr(self, 'align_c4'):
            c4 = self.align_c4(c4)
            c4 = self.align_c4_bn(c4)
            c4 = self.align_c4_relu(c4)
...
if __name__ == '__main__':
    cfg = {
        ...
        'dilations': [6, 12, 18],
        # 'model_path': "logs/best.pth", 改正对于myunimatchsemiactive

        'model_path': "myEH/d_hrnet/my_mixsemiactive_xin/best_epoch_weights.pth",
