...
class PSA(nn.Module):
    def __init__(self, channel, reduction=4):
        ...
        self.conv_q = nn.Conv2d(channel, channel // reduction, 1)
        self.conv_k = nn.Conv2d(channel, channel // reduction, 1)
        self.conv_v = nn.Conv2d(channel, channel, 1)
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, x):
        b, c, h, w = x.size()
        q = self.conv_q(x).view(b, -1, h * w)        # [B, C//r, HW]
        k = self.conv_k(x).view(b, -1, h * w)        # [B, C//r, HW]
...
class DeepLabV3Plus(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg['backbone'] in ["resnet50", "resnet101"]:
            ...
            # >>> PSA
            self.psa = PSA(self.high_channels)
...
if __name__ == '__main__':
    cfg = {
        'nclass': 6,
        'pretrained': False,
        'backbone': 'hrnet',  # 只有 HRNet 时才会启用 PSA
