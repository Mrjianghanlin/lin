from models.new_network.utils import IntermediateLayerGetter
from models.new_network._deeplab import DeepLabHead, DeepLabHeadV3Plus, DeepLabV3
from models.new_network.backbone import xception, model_v3, shffnet, hrnetv2, resnet, mobilenetv2
...
def _segm_mobilenetv2(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    if output_stride==8:
        aspp_dilate = [12, 24, 36]
    else:
        aspp_dilate = [6, 12, 18]
    ...
    # backbone.mid_level_features = backbone.features[4:8]
...
def _segm_mobilenetv3(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    if output_stride == 8:
        aspp_dilate = [12, 24, 36]
    else:
        aspp_dilate = [6, 12, 18]

    backbone = model_v3.mobilenet_v3(pretrained=pretrained_backbone, num_classes=num_classes)
...
def _segm_shufflenet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    backbone = shffnet.shufflenet(pretrained=pretrained_backbone, num_classes=num_classes)

    # inplanes = 2048
    # low_level_planes = 256
    inplanes = 2048
    low_level_planes = 244

    if name == 'deeplabv3plus':
        return_layers = {'conv5': 'out', 'stage2': 'low_level'}
        classifier = DeepLabHeadV3Plus(inplanes, low_level_planes, num_classes, aspp_dilate)
    elif name == 'deeplabv3':
        return_layers = {'layer4': 'out'}
        classifier = DeepLabHead(inplanes, num_classes, aspp_dilate)
...
def _load_model(arch_type, backbone, num_classes, output_stride, pretrained_backbone):
    ...
    if backbone=='mobilenetv2':
        model = _segm_mobilenetv2(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
        ...
        model = _segm_mobilenetv3(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
...
def deeplabv3plus_mobilenetv3(num_classes=21, output_stride=8, pretrained_backbone=True):
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3plus', 'mobilenetv3', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
...
def deeplabv3plus_resnext50_32x4d(num_classes=21, output_stride=8, pretrained_backbone=True):
    """Constructs a DeepLabV3+ model with a Xception backbone.
...
def deeplabv3plus_shufflenet(num_classes=21, output_stride=8, pretrained_backbone=True):
    """Constructs a DeepLabV3+ model with a Xception backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3plus', 'shufflenet', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
if __name__ == '__main__':
    deep = deeplabv3plus_shufflenet(6,8,False)
    print(deep)