...
def resnext101_32x8d(num_classes=1000, include_top=True, stride_groups=None):
    groups = 32
    width_per_group = 8
    return ResNet(
        block=Bottleneck,
        blocks_num=[3, 4, 23, 3],   # 注意这里是 ResNeXt101 的层数
        num_classes=num_classes,
        include_top=include_top,
        groups=groups,
        width_per_group=width_per_group,
...
def resnext101_32x8d_backbone(pretrained=False, downsample_factor=8, backbone_path=""):
    if downsample_factor == 8:
        stride_groups = [1, 2, 1, 1]
    elif downsample_factor == 16:
        stride_groups = [1, 2, 2, 1]
    ...
    backbone = resnext101_32x8d(stride_groups=stride_groups)
