...
class DeepLab_cff(nn.Module):
    def __init__(
        ...
        super(DeepLab_cff, self).__init__()
    ...
    def forward(self, x,need_fp=False):
        ...
        if self.backbone_name in [
           ...
           low_level_features,the_three_features,the_three_features, x = self.backbone(x)
        ...
        # x = self.denseaspp(x)
        cff_out = self.cff(the_three_features, the_three_features)
...
if __name__ == '__main__':
    model = DeepLab_cff(num_classes=3,backbone="hrnet", downsample_factor=16, pretrained=True)
