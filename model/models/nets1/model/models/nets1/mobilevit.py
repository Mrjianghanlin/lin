...
#     if pretrained:
#         model.load_state_dict(torch.load("./model_data/mobilenet_v2.pth.tar"))
#         '''
...
def mobile_vit_small(num_classes: int = 1000,pretrained=False):
    ...
    model = MobileViT(model_cfg=config, num_classes=num_classes)
    if pretrained:
            model.load_state_dict(torch.load(r"D:\study\study\new_all_deeplab\model_data\mobilevit_s.pt"))
...
def mobile_vit_small_backbone(model_type="small",pretrained=False):
    backbone = None
    if model_type == "small":
        backbone = mobile_vit_small(pretrained=pretrained)
