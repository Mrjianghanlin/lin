...
from .modules.backbones.seresnet_ibn import se_resnet101_ibn_a
...
from .modules.backbones.resnest import resnest50,resnest101,resnest200,resnest269
...
class DeepV3Plus_CA(nn.Module):
    """
    DeepLabV3+ with various trunks supported
    Always stride8
    """
    # def __init__(self, in_channels, n_classes, backbone, model_path, use_dpc=False, init_all=False,pretrained):
    def __init__(self, in_channels, n_classes, backbone, pretrained,use_dpc=False, init_all=False):
        super(DeepV3Plus_CA, self).__init__()
        ...
        if backbone == 'resnet50_ibn_a':
            self.base_model = resnet50_ibn_a(pretrained=pretrained)
            s2_ch = 256
            high_level_ch = 2048

        if backbone == 'resnet101_ibn_a':
            self.base_model = resnet101_ibn_a(pretrained=pretrained)
            s2_ch = 256
            high_level_ch = 2048
        if backbone == 'resnest50':
            self.base_model = resnest50(pretrained=pretrained)
            s2_ch = 256
            high_level_ch = 2048
        if backbone == 'se_resnet101_ibn_a':
            self.base_model = se_resnet101_ibn_a(pretrained=pretrained)
            s2_ch = 256
            high_level_ch = 2048
        if backbone == 'resnext101_ibn_a':
            self.base_model = se_resnet101_ibn_a(pretrained=pretrained)
            s2_ch = 256
            high_level_ch = 2048
        if backbone == 'resnest269':
            self.base_model = resnest101(pretrained=pretrained)
            s2_ch = 256
            high_level_ch = 2048
        model_path=False
        if model_path:
        ...
        self.CA = CoordAtt(high_level_ch, high_level_ch)
    ...
    def forward(self, x):
        x_size = x.size()
        s2_features, _,_, final_features = self.base_model.forward_features(x)
        final_features = self.CA(final_features)
...
if __name__ == '__main__':
    model = DeepV3Plus_CA(in_channels=3, n_classes=6, backbone="resnest50", pretrained=False)
    print(model)
