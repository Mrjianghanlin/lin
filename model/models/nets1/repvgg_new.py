...
def repvgg_backbone_new(model_type, pretrained=True):
    # Check if the model_type exists in the dictionary
    if model_type not in func_dict:
        raise ValueError(f"{model_type} is not supported!")

    # Create the model using the provided function in the dictionary
    model = func_dict[model_type](deploy=False, use_checkpoint=False)

    # Load the pretrained weights if pretrained is set to True
    if pretrained:
        if model_type == 'RepVGG-B2g4-new':
            pretrained_weights_path = "./model/models/nets1/model_data/RepVGG-B2g4-train.pth"

            # Load only weights that exist in both the pretrained weights and the model
            model_dict = model.state_dict()
            pretrained_dict = torch.load(pretrained_weights_path)
            matching_keys = {k: v for k, v in pretrained_dict.items() if k in model_dict}
            model_dict.update(matching_keys)
            model.load_state_dict(model_dict)
        else:
            raise ValueError(f"No pretrained weights available for {model_type}!")
