...
class DANet_Fusion(nn.Module):
    """用于替换 CAFM 的 DANet 融合模块（双分支跨域融合）"""

    def __init__(self, cnn_channels, swin_channels=256):
        super(DANet_Fusion, self).__init__()
        # 动态对齐 Swin 分支的通道数到 CNN 主干通道数
        ...
        # 初始化 DANet 的两个核心模块
        self.pam = PAM_Module(cnn_channels)
        self.cam = CAM_Module(cnn_channels)
    ...
    def forward(self, cnn_feat, swin_feat):
        ...
        # 3. DANet 双重注意力提取 (并联结构)
        pam_out = self.pam(fused_feat)
        cam_out = self.cam(fused_feat)

        # 4. 空间注意力和通道注意力的结果相加输出
        out = pam_out + cam_out
...
class DeepLab(nn.Module):
    def __init__(
        ...
        if use_swin:
            ...
            # 使用基于 DANet 的融合模块替换原来的 CAFM
            self.danet_fusion = DANet_Fusion(cnn_channels=in_channels, swin_channels=256)
        ...
        # ========== 核心修改：将解码器的特征注意力增强替换为 DANet ==========
        self.cat_conv = nn.Sequential(
            DANet(512),  # 替换原来的 CoordAtt(512, 512) 或 CBAM(512)
    ...
    def forward(self, x, need_fp=False):
        ...
        if self.use_swin and hasattr(self, 'swin_branch'):
            ...
            # 调用 DANet_Fusion 进行跨分支特征增强
            x = self.danet_fusion(x, swin_feat)
