...
# ========================================== #
#           纯粹的 Element-wise Add 融合
# ========================================== #
class Simple_Add_Fusion(nn.Module):
    """
    最简单的直接相加融合策略，没有任何注意力机制。
    """

    def __init__(self, cnn_channels, swin_channels=256):
        super(Simple_Add_Fusion, self).__init__()
        # 1x1 卷积仅用于通道对齐：Swin(256) -> CNN(cnn_channels)
    ...
    def forward(self, cnn_feat, swin_feat):
        ...
        # 2. 空间尺度对齐 (防止由于下采样倍数不一致导致报错)
        if swin_feat_aligned.shape[2:] != cnn_feat.shape[2:]:
            swin_feat_aligned = F.interpolate(
                swin_feat_aligned,
        ...
        # 3. 直接相加 (Element-wise Add)
        return cnn_feat + swin_feat_aligned
...
class DeepLab(nn.Module):
    def __init__(
        ...
        if use_swin:
            ...
            self.add_fusion = Simple_Add_Fusion(cnn_channels=in_channels, swin_channels=256)
        ...
        # ========== 核心修改：移除了解码器中的所有注意力机制 (CoordAtt / CBAM) ==========
